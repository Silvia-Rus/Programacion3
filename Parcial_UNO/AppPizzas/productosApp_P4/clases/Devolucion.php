...
class Devolucion
{
    ...
    public static function DevolucionesConCupones($arrayDevoluciones, $arrayCupones)
    {       
        foreach($arrayCupones as $cupon)
        {
            $idDevolucionEnCupon = Herramientas::SacarValorDeClave($cupon, "_idDevolucion");

            foreach($arrayDevoluciones as $devolucion)
            {

                $idDevolucion = Herramientas::SacarValorDeClave($devolucion, "_id");

                if($idDevolucionEnCupon == $idDevolucion)
                {
                    Devolucion::Imprimir($devolucion);
                    Cupon::Imprimir($cupon);
                }
            }
        }             
    }
    ...
    public static function Imprimir($item)
    {
        ...
        $idPedido = Herramientas::SacarValorDeClave($item, "_idPedido");
        $causa = Herramientas::SacarValorDeClave($item, "_causa");
        $archivo = Herramientas::SacarValorDeClave($item, "_idPedido");
